...
- **Vitals Monitor** muestra en Add/Edit un selector explícito de formato para My HP y My Mana: detección automática, porcentaje (`100%`) o valor actual/máximo (`1000/1000`). La selección se guarda antes de calibrar y Target, Group y Custom permanecen estrictamente en porcentaje.
...
- **Mobile Host sin cuenta** funciona únicamente en la misma red Wi-Fi/LAN, conserva el emparejamiento privado, puede iniciarse automáticamente y publica un nombre mDNS local estable con la IP como alternativa. Si Windows bloquea la conexión, la ayuda explica cómo permitir Vantage en la red privada o firewall local. No requiere una cuenta de Vantage ni ofrece acceso remoto por Internet.
...
La página móvil se sirve desde la PC únicamente en la red local mientras Mobile Host está activo. El token viaja en el fragmento del enlace, no se guarda en los logs del servidor y las APIs requieren ese token. El emparejamiento queda guardado; el teléfono vuelve a conectarse mediante el nombre mDNS local estable o, si no resuelve, la dirección IP mostrada. La sesión deja de estar disponible al detener Mobile Host o cerrar Vantage. EverQuest Live usa una segunda llave exclusiva del QR Wi-Fi, separada del enlace emparejado normal. La captura está apagada en cada inicio y sólo trabaja cuando el teléfono mantiene visible esa pestaña. Con WinEQ2, Vantage reconoce la superficie activa hija o propietaria y captura directamente la ventana del juego; al cambiar de aplicación se pausa para no exponer otra ventana. El teléfono muestra el motivo real si la captura debe esperar.

Vantage se distribuye como un único `Vantage.exe`: no requiere instalación, DLLs sueltas ni una carpeta junto al ejecutable. La configuración, caché, sonidos copiados y grabaciones de mapa se conservan en `%LOCALAPPDATA%\Vantage`. Puedes mover el ejecutable sin arrastrar archivos adicionales.
