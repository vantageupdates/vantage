...
- **Items & Notes** desde la Quick Bar guarda snapshots separados por personaje de `/outputfile inventory`; ese único dump P99 incluye inventario y banco. Permite buscar, filtrar por ubicación, ajustar cantidades, quitar filas, deshacer y restaurar hasta cinco imports anteriores sin modificar el archivo de EverQuest. Las notas se guardan automáticamente fuera del ejecutable. Al escribir `@` aparece un selector de Items, Quests y Zones; la vista previa convierte las referencias en enlaces que abren las fichas internas de Vantage.
