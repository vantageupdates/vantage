...
function openItem(item){
  const body=$('detail-body'),title=node('h2','detail-title',item.name);title.id='detail-title';body.replaceChildren();body.append(node('p','eyebrow','PROJECT 1999 ITEM'),title);
  ...
  const link=node('a','source-link','Open Project 1999 Wiki source (new tab)');link.href=item.wiki;link.target='_blank';link.rel='noreferrer noopener';body.append(link);$('detail').showModal()
}
...
function openSpell(spell){const body=$('detail-body'),title=node('h2','detail-title',spell.name);title.id='detail-title';body.replaceChildren();body.append(node('p','eyebrow','P99 CLASSIC SPELL'),title,node('p','summary',spell.summary||'Bundled P99 spell data'),node('h3','','Classes and available levels'));(spell.levels||[]).forEach(row=>body.append(node('div','effect',`${row[0]} · Level ${row[1]}`)));const link=node('a','source-link','Open Project 1999 Wiki spell page (new tab)');link.href=spell.wiki;link.target='_blank';link.rel='noreferrer noopener';body.append(link);$('detail').showModal()}
