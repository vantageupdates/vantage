# Location Server for Vantage



The location server runs on port: `8424`

Set Vantage `Settings -> Sharing -> Sharing Hostname` to: `ws://localhost:8424` (you can replace `localhost` with your IP or domain name).
...
2. To build and run in Docker, use one of these commands:
   ...
   2. docker:
      1. `docker build -q .`
      2. `docker run --rm -d -p 8424:8424 --name vantage-locserver <image_sha>` (replacing `<image_sha>` with the output of the first command)
