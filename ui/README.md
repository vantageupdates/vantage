...
`VantageUI-v<major.minor.patch>`, for example `VantageUI-v1.44.64`. All XML and
...
independent UI patch version in `ui/release.json`; never reuse a published UI
...
candidate the tag is `vantage-ui-v1.44.64`. Verify public asset sizes and
...
as `uifiles\VantageUI-v1.44.64`. Shared managed-version state is stored beside
