...
`VantageUI-v<major.minor.patch>`, for example `VantageUI-v1.44.85`. All XML and
...
The 1.44.85 spell-name contrast pass places the existing eight dark spell-name
labels over a warm, translucent inset inside each spell gem. Native spell IDs,
geometry, Pieces order, icon lanes, colored perimeters and click targets are
unchanged. Offline structure and contrast checks pass; spell casting, hotkeys,
hover/pressed feedback and the final text appearance still need a live reload.
...
The 1.44.84 Bank layout is 136 by 160 pixels, with eight native bag slots in a
4-by-2 grid, four coin controls in a 2-by-2 grid, and equal Change/Done buttons.
All visible controls fit inside the native frame without overlap. Native bank
bindings, shared placeholders and child order are preserved; no synthetic bag
content mirrors are reintroduced, preserving the earlier ghost-icon repair.
Five-digit coin spacing is checked geometrically, but live bank clicks, item
transfers and native text rendering have not yet been verified in game.

The 1.44.83 urgent Group repair restores the exact known-visible 1.44.80 button
geometry and atlas after the footer move made the actions disappear in game.
Invite/Follow return to (133, 0), Disband/Decline to (206, 0), with matching
64-by-16-pixel faces. Native alias bindings, member order, statistics, bars and
the 1.44.82 compatibility changes are preserved. The separate TargetWindow
runtime warning remains under investigation; this release does not resolve it.
...
unchanged. Regression checks verify native child membership. A subsequent real
reload cleared the inventory error but still reported the TargetWindow warning.
...
Invite/Disband and their native Follow/Decline aliases to the left footer, and
narrows Pet Info to 136 pixels so its health bar and controls have symmetric
padding.
...
candidate the tag is `vantage-ui-v1.44.85`. Verify public asset sizes and
...
as `uifiles\VantageUI-v1.44.85`. Shared managed-version state is stored beside
