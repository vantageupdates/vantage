...
@pytest.mark.parametrize('name,x', [('Invite',129),('Follow',129),('Disband',204),('Decline',204)])
...
def test_button_row_has_five_pixel_gap_and_clears_the_character_heading():
    assert rect(node('Screen','GroupWindow')) == (516,78,284,215)
    assert 204-(129+70) == 5
...
def test_all_button_states_have_matching_native_frames_clear_corners_and_neutral_rims(i,state):
    ...
    assert rect(a.find('Frames')) == (2,2+20*i,70,16)
    assert a.findtext('Frames/Duration') == '1000'
    for x,y in ((0,0),(69,0),(0,15),(69,15)):
...
def test_raised_pressed_hover_and_disabled_faces_are_distinct_with_identical_alpha():
    hashes=set()
    for i in range(5):
        values=b''.join(bytes(pixel(i,x,y)) for y in range(16) for x in range(70))
        hashes.add(hashlib.sha256(values).digest())
        assert all(pixel(i,x,y)[3]==pixel(0,x,y)[3] for y in range(16) for x in range(70))
    assert len(hashes)==5
...
def test_atlas_is_flat_power_of_two_and_has_clear_gutters():
    ...
    for y in range(128):
        for x in range(128):
            if not (2<=x<72 and any(2+20*i<=y<18+20*i for i in range(5))):
