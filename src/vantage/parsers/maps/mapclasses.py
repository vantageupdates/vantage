    ...
    def __init__(self, **kwargs):
        ...
        if self.name == "__you__":
            self.icon = QGraphicsPixmapItem(
                QPixmap(resource_path('data/maps/user.png'))
            )
            self.setZValue(15)
        else:
            self.icon = QGraphicsPixmapItem(
                QPixmap(resource_path('data/maps/otheruser.png'))
            )
            self.setZValue(10)
...
class SpawnPoint(QGraphicsItemGroup):
    ...
    def __init__(self, **kwargs):
        ...
        self.__dict__.update(**kwargs)
        self.setToolTip(
            f'{self.name} · drag to move · double-click to restart · '
        ...
        pixmap = QGraphicsPixmapItem(QPixmap(resource_path('data/maps/spawn.png')))
    ...
    def stop(self):
        ...
        QTimer.singleShot(
            4 * 60 * 1000,
            lambda: self._remove_if_expired(generation))

    def _remove_if_expired(self, generation):
    ...
    def itemChange(self, change, value):
        if (change == QGraphicsItem.GraphicsItemChange.ItemPositionHasChanged
                and hasattr(self, 'location')):
            self.location.x = value.x()
            self.location.y = value.y()
...
class WayPoint:
    ...
    def __init__(self, **kwargs):
        ...
        self.pixmap = QGraphicsPixmapItem(
            QPixmap(resource_path('data/maps/waypoint.png')))
