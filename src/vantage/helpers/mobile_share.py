...
def load_mobile_spell_detail(name):
    ...
    request = Request(
        ...
        headers={"User-Agent": "Vantage/1.44.75"})
...
let timerUid=0,lastConnection='',polling=false,marketDelay,marketRequest=0,spellDelay,spellRequest=0,gameLoading=false,gameObjectUrl='',gameDelay,timerListWasEmpty=null,syncingZone=false,syncingGuild=false,pendingZone='',pendingGuild='',zoneRefreshDelay,guildRefreshDelay,guildData={},zoneData={},questDelay,deferredInstallPrompt=null;let timerStates=new Map();
...
function showInstallHelp(){detailTitle.textContent='Save Vantage to your Home Screen';detailBody.replaceChildren();const intro=node('p','summary','Keep this mobile view one tap away while the current Vantage session is running.');const steps=node('ol','install-steps');const ios=/iphone|ipad|ipod/i.test(navigator.userAgent);for(const text of (ios?['Open this page in Safari.','Tap Share.','Choose Add to Home Screen, then tap Add.']:['Open your browser menu.','Choose Add to Home screen or Install app.','Confirm Add.']))steps.append(node('li','',text));const limit=node('p','limitation','Important: this shortcut belongs to the current private session. If it shows OFFLINE after Vantage restarts, scan the new QR and save the new link.');detailBody.append(intro,steps,limit);detailDialog.showModal()}
...
async function loadGuild(){guildRoot.setAttribute('aria-busy','true');try{guildData=await get('/api/guild');renderGuild();return true}catch(_){showListMessage(guildRoot,'Guild data could not be loaded.');announce(guildStatus,'Guild data could not be loaded.');return false}finally{guildRoot.setAttribute('aria-busy','false')}}
...
async function loadZones(){zoneRoot.setAttribute('aria-busy','true');try{zoneData=await get('/api/zones');renderZones();return true}catch(_){showListMessage(zoneRoot,'Zone information could not be loaded.');announce(zoneStatus,'Zone information could not be loaded.');return false}finally{zoneRoot.setAttribute('aria-busy','false')}}
...
class _ShareHandler(BaseHTTPRequestHandler):
    server_version = "VantageMobile/1.44.75"
