...
def load_mobile_spell_detail(name):
    ...
    request = Request(
        ...
        headers={"User-Agent": "Vantage/1.44.108"})
...
def _mobile_wiki_numeric_stats(stats_text):
    """Promote core weapon/card values from the desktop Wiki cache."""
    source = str(stats_text or "")
    values = {}
    for key, pattern in _MOBILE_WIKI_STAT_PATTERNS:
...
def _merge_mobile_item_detail(item, wiki_detail):
    ...
    complete_stats = dict(detail["stats"])
    for key, value in _mobile_wiki_numeric_stats(stats_text).items():
        complete_stats.setdefault(key, value)
    cached_numeric = wiki_detail.get("numeric_stats", {})
    if isinstance(cached_numeric, dict):
        for key in ("dmg", "dly", "ac", "hp", "mana"):
            try:
                value = int(cached_numeric[key])
...
<script>
const byId=id=>document.getElementById(id),sessionTokenKey='vantageMobileSessionToken',pairingKey='vantageMobilePairingV1',dataCachePrefix='vantageMobileData:',dataCacheIndexKey='vantageMobileDataIndex';
function validPairingToken(value){return /^[A-Za-z0-9_-]{20,128}$/.test(String(value||''))}function readPairing(){try{const saved=JSON.parse(localStorage.getItem(pairingKey)||'null');if(saved&&saved.host===location.host&&validPairingToken(saved.token))return saved;const legacy=localStorage.getItem(sessionTokenKey)||'';return validPairingToken(legacy)?{host:location.host,token:legacy,lastConnected:0}:null}catch(_){return null}}function rememberPairing(value){if(!validPairingToken(value))return;try{localStorage.setItem(sessionTokenKey,value);localStorage.setItem(pairingKey,JSON.stringify({host:location.host,token:value,lastConnected:Date.now()}))}catch(_){}}function forgetPairing(){try{localStorage.removeItem(sessionTokenKey);localStorage.removeItem(pairingKey)}catch(_){}}let token=location.hash.slice(1),savedPairing=readPairing();try{if(validPairingToken(token)){rememberPairing(token)}else token=savedPairing?savedPairing.token:''}catch(_){}
...
const phases={idle:'READY',respawn:'RESPAWN',combat:'COMBAT',available:'AVAILABLE'},statLabels={dmg:'DMG',dly:'DLY',ac:'AC',hp:'HP',mana:'Mana',astr:'STR',asta:'STA',adex:'DEX',aagi:'AGI',aint:'INT',awis:'WIS',acha:'CHA',mr:'MR',fr:'FR',cr:'CR',dr:'DR',pr:'PR',attack:'ATK',haste:'Haste',regen:'Regen',manaregen:'Mana regen'};
...
async function get(path){const r=await fetch(path,{cache:'no-store',headers:{Authorization:'Bearer '+token}});if(!r.ok){const failure=Error(String(r.status));failure.status=r.status;throw failure}const data=await r.json();writeCached(path,data);return data}
...
function setConnection(text,offline=false,message='',actionRequired=false){if(text===lastConnection)return;lastConnection=text;state.textContent=text;state.classList.toggle('offline',offline);state.classList.toggle('disconnected',actionRequired);if(actionRequired){clearTimeout(connectionAnnouncementDelay);pendingPoliteConnection='';connectionStatus.textContent='';announce(connectionAlert,message||text)}else{connectionAlert.textContent='';announceConnectionPolitely(message||text)}}
...
function announceGuild(text,deferred=false){clearTimeout(guildAnnounceDelay);if(deferred){guildAnnounceDelay=setTimeout(()=>announce(guildStatus,text),500);return}announce(guildStatus,text)}
function renderGuild(deferAnnouncement=false){syncGuildOptions(guildData);const view=byId('guildView').value,query=byId('guildSearch').value.trim().toLowerCase(),rows=Array.isArray(guildData[view])?guildData[view]:[];const filtered=rows.filter(row=>JSON.stringify(row).toLowerCase().includes(query)),visible=query?filtered:filtered.slice(0,250),total=view==='standings'?Number(guildData.standings_total||rows.length):rows.length;guildNote.textContent=String(guildData.guild||'No guild selected')+' · '+String(guildData.status||'Public guild data')+(view==='standings'?' · '+total+' members':'');if(!filtered.length){const message=guildData.loading?'Loading DKP and guild records…':guildData.connected?'No rows match this view.':'Choose a saved guild above. Add guilds first in Vantage on the PC.';showListMessage(guildRoot,message);announceGuild(message,deferAnnouncement);return}stableReplace(guildRoot,'guild:'+JSON.stringify([view,visible]),...visible.map(item=>{const row=node('li','card data-card'),title=node('h3','');if(view==='loot'){const link=node('button','card-button item-link',String(item.item||'Item'));link.type='button';link.addEventListener('click',()=>openLinkedItem(item.item,item.wiki_url));row.append(link);title.textContent=String(item.character||'—')+' · '+String(item.dkp||'—')+' DKP';row.append(title,node('p','meta',[item.date,item.raid].filter(Boolean).join(' · ')));return row}title.textContent=String(item.name||item.item||'Guild record');row.append(title);const grid=node('div','data-grid');for(const [key,value] of Object.entries(item)){if(['name','item','wiki_url','url'].includes(key))continue;grid.append(node('span','',key.toUpperCase()+' · '+String(value)))}row.append(grid);if(item.url){const link=node('a','source-link','Open guild sheet');link.href=item.url;link.target='_blank';link.rel='noopener noreferrer';link.setAttribute('aria-label','Open guild sheet '+String(item.name||'')+'; opens in a new tab');row.append(link)}return row}));announceGuild((query?filtered.length:Math.min(filtered.length,250))+' of '+filtered.length+' guild rows shown',deferAnnouncement)}
...
async function chooseGuild(target){if(!target||syncingGuild||pendingGuild)return;clearTimeout(guildRefreshDelay);pendingGuild=target;guildNote.textContent='Switching guild…';showListMessage(guildRoot,'Loading DKP and guild records…');announceGuild('Loading DKP and guild records…');try{await post('/api/browser/action',{action:'guild',target});settleGuild(target)}catch(_){pendingGuild='';renderGuild();announceGuild('Guild could not be selected.')}}
...
guildSelect.addEventListener('change',()=>chooseGuild(guildSelect.value));byId('guildView').addEventListener('change',()=>renderGuild());byId('guildSearch').addEventListener('input',()=>renderGuild(true));byId('zoneView').addEventListener('change',renderZones);byId('zoneSearch').addEventListener('input',renderZones);zoneSelect.addEventListener('change',()=>chooseZone(zoneSelect.value));zoneReload.addEventListener('click',()=>chooseZone(zoneSelect.value));byId('questSearch').addEventListener('input',()=>{clearTimeout(questDelay);questDelay=setTimeout(loadQuests,180)});
...
class _ShareHandler(BaseHTTPRequestHandler):
    server_version = "VantageMobile/1.44.108"
...
class MobileShareController(QObject):
    ...
    def _shutdown_session(self, *, explicit):
        ...
        if self._server:
            ...
            self._server = None
        if (server_thread is not None and server_thread.is_alive() and
                server_thread is not threading.current_thread()):
